...
def do_track(cfg, model):
    ...
    cudnn.benchmark = False  # 不讓 cuDNN 自選 kernel
    cudnn.deterministic = True  # 強制確定性演算法
    torch.manual_seed(42)  # CPU 亂數鎖定
    torch.cuda.manual_seed(42)  # GPU 亂數鎖定
    ...
    folder_name = getattr(cfg.track, "folder_name", "track_results")
    results_folder = os.path.join(file_name, folder_name)
    ...
    # start evaluate
    evaluator.evaluate(model, cfg.track.fp16, results_folder)
    ...
    if cfg.track.mot20:
        gtfiles = glob.glob(
            os.path.join("../MOT20/train", "*/gt/gt{}.txt".format(gt_type))
        )
    else:
        gtfiles = glob.glob(
            os.path.join("../MOT17/train", "*/gt/gt{}.txt".format(gt_type))
        )
