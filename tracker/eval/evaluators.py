...
class MOTEvaluator:
    ...
    def evaluate(self, model, half=False, result_folder=None):
        """
        ...
        for cur_iter, batch_data in enumerate(progress_bar.tqdm(self.dataloader)):
            with torch.no_grad():
                # init tracker
                ...
                if self.args.track.is_public:
                    if frame_id == 1:
                        det_path = os.path.join(
                            batch_data[0]["file_name"][:-10].replace("img1", "det"),
                            "det.txt",
                        )
                        video_dets = np.loadtxt(
                            det_path, dtype=np.float32, delimiter=","
                        )
                ...
                if frame_id == 1:
                    ...
                    if hasattr(tracker, "glra_stats") and len(results) != 0:
                        ...
                        s = tracker.glra_stats
                        lf = (
                            np.array(s["lost_frames"])
                            if s["lost_frames"]
                            else np.array([0])
                        )
                        print(
            ...
            if det_instances is not None:
                ...
                for t in online_targets:
                    tlwh = (
                        t.tlwh
                        if not (self.args.track.sort or self.args.track.ocsort)
                        else t[:4]
                    )
                    tid = (
                        t.track_id
