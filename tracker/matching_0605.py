...
def gpr_predict_bbox(track, target_frame, min_obs=3):
    ...
    sigma_px  : float – average positional uncertainty in pixels (back-transformed
                from z-score space), used by glra_distance for adaptive thresholding.
    ...
    try:
        ...
        # predict() with return_std=True gives std in z-score space;
        # multiply by original std to convert back to pixel units.
        ...
        # Back-transform sigma to pixel space and average over x/y
        sigma_px = (float(sigma_cx_z[0]) * cx_std + float(sigma_cy_z[0]) * cy_std) / 2.0
    except Exception:
        return None
